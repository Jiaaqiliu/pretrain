...
\end{equation}
This measures the \emph{effective dimensionality}: how many ``$\sigma_1$-sized'' directions the matrix uses. Crucially, SR requires only the Frobenius norm ($O(mn)$) and the top singular value (power iteration, $O(mn)$); no full SVD is needed.
