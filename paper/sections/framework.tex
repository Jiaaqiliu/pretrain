...
At initialization, Marchenko-Pastur theory gives $\mathrm{SR}_{\mathrm{init}} \approx mn/(\sqrt{m}+\sqrt{n})^2 \approx 0.4d$. The empirical convergence $\mathrm{SR}_{\mathrm{final}} \approx 0.055d$ (Section~\ref{sec:results}) implies:
\begin{equation}
    \Delta H_2 \;\approx\; \log\!\bigl(\mathrm{SR}_{\mathrm{final}}/\mathrm{SR}_{\mathrm{init}}\bigr) \;=\; \log(0.055/0.4) \;\approx\; -2.0 \;\text{nats}.
    \label{eq:delta_entropy}
\end{equation}
This is not a metaphor: training universally removes ${\approx}2$ nats of R\'{e}nyi-2 spectral entropy per layer, regardless of model scale (190M--32B) or architecture family. The Universal Compression Ratio $\mathrm{UCR} = \mathrm{SR}_{\mathrm{final}}/\mathrm{SR}_{\mathrm{init}} \approx 0.14$ is the fraction of initial spectral entropy retained.
