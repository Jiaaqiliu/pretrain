...
Figure~\ref{fig:dashboard} shows the monitoring dashboard in action on a real training run (Pythia-410M, $\alpha$-guided schedule). The three panels---loss+LR, $\alpha$ trajectory, and SR/$d$ trajectory---provide a complete instrument panel. In this run, $\alpha$ decreases monotonically (no reversal detected), confirming that the 410M model with $D/N = 740$ has sufficient data to achieve structural maturity without triggering early decay.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.85\columnwidth]{figures_v2/fig09_dashboard.pdf}
    \caption{\textbf{Spectral monitoring dashboard} for a real training run (Pythia-410M, $\alpha$-guided, FineWeb-Edu). Top: loss and learning rate. Middle: $\alpha$ trajectory with reversal detection (green = healthy, no reversal detected). Bottom: SR/$d$ compression dynamics. The three panels together provide real-time structural diagnostics that complement the loss curve.}
    \label{fig:dashboard}
\end{figure}
...
\paragraph{Downstream evaluation scale.} The 3-way schedule downstream evaluation (\S\ref{sec:schedule}) validates at two scales (410M and 1B) on 5 benchmarks, showing the $\alpha$-guided advantage amplifies from $+1.95\%$ to $+2.56\%$. Evaluation at 7B+ with more comprehensive benchmark suites (MMLU, GSM8K) would further strengthen the prescriptive claim, though the consistent amplification trend suggests the advantage continues to grow.
