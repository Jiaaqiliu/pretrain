...
\paragraph{Scale ceiling.} Our measurements now extend to 13B parameters, with 32B and 70B measurements in progress. Whether SR/$d \approx 0.055$ holds at 70B+ remains unverified, though the asymptotic model (SR/$d = 0.04 + 0.61/\sqrt{d}$) predicts convergence to 0.04 for very large $d$.

\paragraph{Architecture scope.} We validate across GPT-NeoX, LLaMA, and OLMo-2---all dense decoder-only transformers. MoE and encoder-decoder architectures are untested.

\paragraph{Size-dependent $\tau$.} The Structural Chinchilla formula requires a model-size correction at 13B+ scale, which we are currently characterizing with additional measurements (32B, 70B). Until this correction is calibrated, the formula should be applied with caution above 7B parameters.

\paragraph{Proxy training.} The $\alpha$-guided experiment (\S\ref{sec:schedule}) uses random tokens as a proxy. A real-data experiment is currently running to validate that the spectral dynamics and scheduling gains transfer to natural language with downstream evaluation.
