...
(the same split-table used by the Kaggle training set) to a large teacher model
(default Nemotron-3-Super-120B-FP8), filters rollouts by correctness +
length, and emits a JSONL file that subsequent SFT stages consume via
``data/sources.yaml``.
...
by default (via ``python -m agent_evolve.model.runners.stages.teacher_distill``),
...
def _run_real_synth(workspace: Any, stage: dict) -> tuple[Path, dict[str, Any]]:
    ...
    use_subprocess = os.environ.get("AE_SYNTH_SUBPROCESS", "1") != "0"
    if use_subprocess:
...
def _run_real_synth_inproc(cfg: dict) -> None:
    ...
    prompts_df = pd.concat(sampled).reset_index(drop=True)
...
def _load_real_synth_config(workspace: Any, stage: dict) -> dict:
    ...
    return {
        "teacher_model_path": teacher_cfg.get(
            "teacher_model_path",
            "/fsx/models/NVIDIA-Nemotron-3-Super-120B-A12B-FP8",
        ),
