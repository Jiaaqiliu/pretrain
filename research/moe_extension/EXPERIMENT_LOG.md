...
> 基于 Phase 0-2 + 逐层 + dense 补测的结果，对深层规律的解读、可解释视角、以及对训练的启发。

### 1. 三个层次的重新理解

**(1) α 不是"训练质量诊断"，而是"单矩阵容量"的结构量。**  
HT-SR 经典解读 (Martin & Mahoney): α∈[2,4] 是训练良好甜区，α>6 欠训练 (接近随机矩阵 Marchenko-Pastur)，**α<2 被解读为"过度相关/过拟合"**。但 OLMoE 专家 α=1.46，模型却健康强大、毫不过拟合。统一 α-vs-width 规律说明: **决定 α regime 的是单矩阵宽度/自由度，不是训练好坏、也不是 MoE/dense 之分**。α 应从"诊断指标"重新定位为"容量的函数"。

**(2) MoE 没有 dense 那种结构相变 —— "born solid"。**  
Dense: α 从随机态 ~6 掉到 ~3 甚至反转回升 (退火/结晶式)。MoE: 专家从 init 就在 α≈1.46 全程不动，只有 ψ 在涨。物理类比: **dense 是"先熔化再凝固"，MoE 是"出生即玻璃态"** —— 结构在 init 时被约束锁定，训练只旋转不重塑。

**(3) 信息量从 α 转移到 EPR/ψ。**  
α 平了之后，MoE 真正的动力学藏在 EPR (U 型: 均衡化→特化) 和 ψ (单调升: 功能锐化) 里。**dense 的"α 反转预警"在 MoE 完全失效，需换观测量。**

### 2. 揭示的深层规律

**规律 A — 容量瓶颈 → 谱压缩，架构无关的普适律。**  
每个 OLMoE 专家只见 top-8/64 token，在受限子空间内吸收丰富梯度 → 奇异谱无空间铺开 → 相关性挤进少数方向 → 重尾 → 低 α。Phase 2 完美阶梯 (1024→6400→14336 ⇒ 1.46→3.03→4.00) 是直接证据。**把 MoE 的 α<2 从"过拟合"重新解释为"信息瓶颈的必然" —— 本工作最有论文价值的论点。**

**规律 B — 两种"特化"正交，解开一个表面矛盾。**  
DeepSeekMoE 论证"细粒度专家(更多更窄)→ 更强特化"(组合灵活性)。但谱发现相反: OLMoE(64专家)谱近乎相同(σ_α≈0.02)，Mixtral(8专家)高度分化(σ_α≈0.85)。不矛盾，因为是两种特化:
- **功能特化** (DeepSeekMoE): 专家*处理哪些* token/知识 —— 路由/语义层
- **结构特化** (本工作测的): 专家权重矩阵的*谱复杂度* —— 权重层

→ **细粒度 MoE 把特化外包给"组合/路由"层，让单专家保持可互换的同质"原语"; 粗粒度 MoE 把特化压进"权重"本身。** 两种归纳偏置，无优劣。

**规律 C — 跨架构的功能压缩层级。**  
q/k(α≈1.1) < v/o(α≈1.27) < FFN，dense 和 MoE 都成立。暗示**功能决定谱**的层级: 相似度计算(q/k)需最强相关集中(与 attention 低秩的已知结论一致)，值变换(v/o)次之，特征混合(FFN)需最多展开。MoE 只是把整个层级平移进 Lévy 区(gap 1.26→0.41)，没重排序。

**规律 D — 弹道式收敛 (β>1) 暗示"被驱动"的动力学。** ⚠️ 证据较弱(单模型轨迹)  
Dense 弛豫是 KWW 拉伸指数(β<1，玻璃态、多时间尺度)。MoE 的 SR/d 是压缩指数(β=1.85，弹道式)，通常出现在**被外力驱动的活性物质**而非平衡玻璃。解释: **load-balancing loss 充当外部驱动力**，确定性地推系统向目标，路由约束"线性化"了动力学。

### 3. 可解释视角

| 视角 | 解释什么 | 关键句 |
|------|---------|--------|
| HT-SR / 随机矩阵论 | α 绝对值与 regime | 但需修正: α<2 ≠ 过拟合 |
| 信息瓶颈 (Tishby) | 为何窄专家被迫重尾 | 容量约束 → 相关性集中 |
| 统计力学 / 相变 | born-solid vs melt-refreeze | MoE 玻璃态淬火 vs dense 退火结晶 |
| 重整化群 / 标度不变 | power-law = 无特征尺度 | 重尾即跨尺度相关 |
| 有效维度 / 彩票假设 | SR/d 是有效自由度 | 专家分化 = 有效维度分化 |
| 活性物质 / 被驱动系统 | β>1 弹道收敛 | 辅助损失 = 外部驱动场 |

最有力的组合: **信息瓶颈 + RMT** —— 用瓶颈解释"为何 α<2 是必然而非病态"，用 RMT 提供可测量的 α/SR/d 工具，直接攻击 HT-SR 把 α<2 当病态的传统解读。

### 4. 对训练的启发和指示

**实操 (可直接用):**
1. **MoE 别再监控 α** (它是死的)。建 **EPR/ψ 仪表盘**: EPR 异常上升 = 专家坍缩/负载失衡预警; ψ 停滞 = 特化没发生。
2. **路由几何 step 5K 就冻结** → (a) router 初始化极重要; (b) 后期可冻结/降 router 学习率省算力; (c) 干预路由须在极早期。
3. **SR/d 公式用 min(shape) 修正** (intermediate>hidden 的瘦长矩阵偏 ~75%)，作跨架构 health-check 补丁。

**设计 (指导架构):**
4. **专家宽度是 α regime 旋钮**。若信 HT-SR 的 α∈[2,4] 甜区，OLMoE 的 α=1.46 是"过压缩"。→ **可测假设: 存在最优专家宽度(也许 intermediate≈4000-6000)，在 α 甜区与参数效率间平衡。** Phi-3.5 (α=3.03) 恰落甜区，值得追问是否更"健康"。
5. **粒度是"权重特化 vs 路由特化"的杠杆**: 要可互换/易剪枝合并 → 细粒度; 要强单专家能力/模块化复用/专家级可解释性 → 粗粒度。
6. **"born solid" → MoE 对专家初始化的敏感度可能高于 dense** (谱结构早定型，init 决定 regime)。

**最值得做的下一实验:**  
测**专家宽度扫描下的下游能力**(而非只测谱)。若 α 甜区假设成立，固定总参数扫 expert width，应在 α≈2-3 处看到能力拐点。这能区分"α 是纯结构量"与"给定 width 下 α 微小偏移仍编码训练质量"两种解读 —— 目前数据只能说 α 主要由 width 决定，尚未排除后者。

**一句话总结:** 本工作把 α 从"训练质量的温度计"重新定位为"单矩阵容量的状态方程"，并揭示 MoE 通过窄专家把整个网络压进重尾 Lévy 区、把特化外包给路由层 —— 一种与 dense 根本不同的、被辅助损失驱动的"玻璃态"训练机制。
