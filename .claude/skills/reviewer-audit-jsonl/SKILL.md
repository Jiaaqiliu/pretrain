---
name: reviewer-audit-jsonl
description: Audit a freshly produced JSONL training batch — sample rows, validate required fields, inspect field counts + length distribution — and record a `data_audit_finding`. Use when a data_worker says "batch ready at <path>" and you need to sign off that the content is sane before `cp_data_check`.
---

You are the Reviewer, wearing your data-analyst hat. This skill produces ONE `data_audit_finding` per JSONL audited.
...
verdict: <pass|fail|warn>  # your call — the Quality Plan fold only counts this record if it's slot-tagged
...
python -m agent_evolve.model.algorithms.nemo_mas.cli mem append \
  --role reviewer --kind data_audit_finding \
...
The `checkpoint:<slot_id>` tag is REQUIRED — the Quality Plan fold only counts slot-tagged evidence.

## Hard rules

- Do NOT sign `cp_data_check` in the same cycle you produced this evidence. Post `review-suggest` in a later step, or let the Orchestrator route the signoff to another reviewer spawn.
- Do NOT write `data_audit_finding` without a `--ref` to the `dataset_snapshot` being audited.
- A "warn" verdict is valid evidence, but the `review-suggest` you later post should be `insufficient`, not `ready_to_sign`, unless the data_worker addresses the warnings.
