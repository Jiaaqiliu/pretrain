...
4. For each rerun, after it completes: spawn an Reviewer (in your
   final response, request this — Trainer doesn't spawn
   Analysts) to run eval and write `eval_report`.

   *Note*: this skill PRODUCES the training_runs and the
   `cv_result`. Trainer doesn't run the eval itself — that's
   Reviewer's tool. So the flow is: launch reruns → wait for
   completion → write a `cv_result` placeholder that lists the
   training_run_ids and asks Orchestrator to spawn Reviewer evals.

   Once Reviewer eval_reports are in, a follow-up Trainer spawn
...
Trainer spawn, NOT a mem_link patch — refs are append-only and
...
body: |
  ...
  Promotion recommendation:
    ...
    If unstable: DO NOT PROMOTE — Planner should propose
...
- Do NOT "rerun eval with different seeds" — eval is deterministic
  at temp=0.0; that's a no-op and Reviewer should refuse.
