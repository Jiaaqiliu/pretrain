...
DataWorker can directly act on.
...
7. For each gap, propose concrete next-batch params (these go in
   the body so DataWorker can execute directly).
...
body: |
  ...
  Proposed next batch (DataWorker reads this):
  ...
  Acceptance criteria (Reviewer will re-audit):
    - Schema valid: 100%
...
- Do NOT write a `data_gap` for `format_error` failures — those are
  recipe issues (boxing discipline). Planner proposes a recipe
  change, not more data.
- Do NOT write a `data_gap` without a concrete proposed-next-batch
  spec. "We need more data" is not actionable — DataWorker will
  refuse it.
...
- Do NOT write more than 3 `data_gap`s per eval. If you have more
  ...
  Planner + Orchestrator prioritize.
