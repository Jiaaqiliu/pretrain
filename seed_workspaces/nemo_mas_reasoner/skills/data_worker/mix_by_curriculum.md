...
Planner `recipe_proposal`). This produces the final
`data/final/train.jsonl` that Trainer trains on.
...
- Do NOT silently mutate `data/mix.yaml` or `data/curriculum.yaml`
  — those are inputs from Planner's `recipe_proposal`. Read-only
  here.
...
- Do NOT overwrite `data/final/train.jsonl` if the new snapshot
  shows < 95% of the previous row count UNLESS Planner's proposal
