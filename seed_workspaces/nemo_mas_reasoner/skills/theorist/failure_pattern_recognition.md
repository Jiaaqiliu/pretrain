...
3. Apply the heuristic priority order. When multiple families
   ...
   e. **`answer_extraction_fail`** — usually an eval-side bug.
      Investigate via `probe_benchmark_format` (Analyst), not via
      training change.

   f. **`eval_runtime_error`** — surface as `failed_attempt` and
      escalate to Engineer. Not a Theorist concern.
...
family to attack, which then feeds into one of the other Theorist
...
body: |
  ...
  Smallest experiment to test:
    spawn engineer to rerun the prior recipe (rec_X) with LoRA
