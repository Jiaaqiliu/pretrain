...
**Consequence for self-distill (DataEngineer / SolverDistiller):**
...
**Implication for Theorist + DataEngineer**: do not over-fit to these
...
Analyst's `eval_report` records SHOULD break down errors by bucket
and by category — this is what Theorist reads to decide what to
change.
...
- **Deterministic eval blurs CV semantics**: rerunning the SAME
  ...
  not the eval seed. See `skills/engineer/cross_validate_recipe`.
